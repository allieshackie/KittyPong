...
COGPhysics::COGPhysics(GameObject* pGO, bool bGenerateCollisionEvents, exVector2 pVelocity)
	: Component(pGO)
	, mGenerateCollisionEvents(bGenerateCollisionEvents), mVelocity(pVelocity), normal({0,0})
{
}
...
bool COGPhysics::IsColliding(COGPhysics* pOther)
{
	if (mCircleShape != nullptr && pOther->mBoxShape != nullptr)
	{
		...
		COGTransform *ball = mGO->FindComponent<COGTransform>(ComponentType::Transform);
		COGCircleShape *ballShape = mGO->FindComponent<COGCircleShape>(ComponentType::CircleShape);
		COGTransform *otherTransform = pOther->mGO->FindComponent<COGTransform>(ComponentType::Transform);
		COGBoxShape *otherShape = pOther->mGO->FindComponent<COGBoxShape>(ComponentType::BoxShape);
		exVector2 closest = {max(otherTransform->GetPosition().x, min(ball->GetPosition().x, otherTransform->GetPosition().x + otherShape->GetWidth())),
							max(otherTransform->GetPosition().y,min(ball->GetPosition().y, otherTransform->GetPosition().y + otherShape->GetHeight())) };

		exVector2 deltaPos = { ball->GetPosition().x - closest.x, ball->GetPosition().y - closest.y };		// get distance from closest point to ball
		float distance = (pow(deltaPos.x, 2) + pow(deltaPos.y, 2));		// get distance

		if (distance - 2 < ballShape->GetRadius() * 2) {	// if distance is lower than radius
			normal = { deltaPos.x / distance, deltaPos.y / distance };
			Score *score = pOther->mGO->FindComponent<Score>(ComponentType::Score);
